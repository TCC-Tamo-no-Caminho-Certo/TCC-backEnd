...
Router.route('/user')
  ...
  .patch(auth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (name || surname) {
        ...
        var userSurname = user.get("surname");
        await lucene.delete(userId)
        await lucene.add(userId, name ? name : userName + " " + surname ? surname : userSurname )
      }
  ...
  .delete(auth, async (req: Request, res: Response) => {
    ...
    try {
      const [user] = await User.find({ user_id: _user_id })
      await user.verifyPassword(password)
      await lucene.delete(user.get("user_id"))
      await user.delete()
