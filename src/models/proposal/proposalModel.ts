...
export default class Proposal {
  ...
  async insert() {
    ...
    await trx('user_proposal').insert({ user_id: this.user_id, proposal_id, permission: 'owner' })
 ...
 */
  async update({ title, version, status, categories }: UpdateProposalObj) {
    ...
    title || version ?
      await this._update.titleAndVersion(this.id_proposal, title, version, trx) :
    ...
    if (categories) {
      ...
      for (let i in categories_id) await trx('category_proposal').insert({ category_id: categories_id[i], proposal_id: this.id_proposal })
    }
  ...
  static async getProposal(user_id: number, id_proposal: number) {
    const proposal_info = await db('proposal_view').where({ id_proposal }).then(row => row[0] ? row : null)
